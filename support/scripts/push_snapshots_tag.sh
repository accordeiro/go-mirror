...
then
    ...
    git push --delete -q https://${GITHUB_TOKEN}@github.com/${CIRCLE_PROJECT_USERNAME}/${CIRCLE_PROJECT_REPONAME} snapshots || true
    git tag snapshots -a -m "Generated snapshots from Circle CI for build ${CIRCLE_BUILD_NUM} on $(date --utc +'%F-%T')"
    git push -q https://${GITHUB_TOKEN}@github.com/${CIRCLE_PROJECT_USERNAME}/${CIRCLE_PROJECT_REPONAME} --tags
fi
